...
class ChannelEmbed(discord.Embed):
    def __init__(self, channel: discord.TextChannel):
        ...
        self.description = (
            f"**Category**: {channel.category.mention if channel.category else ''}\n"
        )
        category_overwrites = channel.category.overwrites if channel.category else {}
        category_string = "\n".join(get_overwrite_string(category_overwrites))
        category_chunk = [
            category_string[i : i + chunk_size]
            for i in range(0, len(category_string), chunk_size)
        ]

        for i, chunk in enumerate(category_chunk):
            self.add_field(
                name=f"Category Overwrites {f'{i+1}' if len(category_chunk)>1 else ''}",
                value=chunk,
                inline=False,
            )

        channel_overwrites = (
            channel.overwrites if hasattr(channel, "overwrites") else {}
        )
        channel_string = "\n".join(get_overwrite_string(channel_overwrites))
        channel_chunk = [
            channel_string[i : i + chunk_size]
            for i in range(0, len(channel_string), chunk_size)
        ]

        for i, chunk in enumerate(channel_chunk):
            self.add_field(
                name=f"Channel Overwrites {f'{i+1}' if len(channel_chunk)>1 else ''}",
                value=chunk,
                inline=False,
            )
