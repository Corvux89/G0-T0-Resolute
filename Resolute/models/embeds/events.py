...
class MemberLeaveEmbed(discord.Embed):
    def __init__(self, player: Player):
        super().__init__(
            title=f"{str(player.member)} ( {f'`{player.member.nick}`' if player.member.nick else 'No nickname'}) has left the server"
        )
        ...
        if host_str is not None:
            self.add_field(name=f"Hosting Arenas", value=host_str, inline=False)
