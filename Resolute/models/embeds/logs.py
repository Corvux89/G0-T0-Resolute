...
class LogEmbed(discord.Embed):
    def __init__(self, log_entry: DBLog, show_values: bool = False, **kwargs):
        ...
        self.set_thumbnail(
            url=(
                log_entry.player.member.display_avatar.url
                if log_entry.player.member
                else THUMBNAIL
            )
        )
        self.set_footer(
            text=f"Logged by {log_entry.author.member.name} - ID: {log_entry.id}",
        ...
        self.description += (
            f"**Character**: {log_entry.character.name}\n"
            if log_entry.character
            else ""
        )

        self.description += (
            f"**Faction**: {log_entry.faction.value}\n" if log_entry.faction else ""
        )
...
class LogStatsEmbed(discord.Embed):
    def __init__(self, player: Player, player_stats: {}, first: bool = True):
        ...
        if first:
            self.description = (
                f"**Charcters (active / total)**: {len([x for x in player.characters if x.active == True])} / {len(player.characters)}\n"
            ...
            self.add_field(
                ...
                value="\n".join(
                    [
                        f"{a.replace('Activity ', '')}: {v}"
                        for a, v in player_stats.items()
                        if "Activity" in a
                    ]
                ),
            )
...
class LogHxEmbed(discord.Embed):
    def __init__(self, player: Player, logs: list[DBLog] = []):
        super().__init__(
            title=f"Log history - {player.member.name}", color=discord.Color.random()
        )
        ...
        for log in logs:
            value = (
                f"**Author**: {log.author.member.mention if log.author.member else '`Not found`'}\n"
            ...
            self.add_field(
                name=f"Log # {log.id} - <t:{log.epoch_time}>", value=value, inline=False
            )
