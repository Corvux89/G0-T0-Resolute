...
class NPCEmbed(discord.Embed):
    def __init__(
        self, guild: PlayerGuild, npcs: list[NPC] = [], primary_npc: NPC = None
    ):
        super().__init__(title=f"Manage NPCs", color=discord.Color.random())
        ...
        npc_list = "\n".join(
            [f"`{n.key}`: {n.name}{'*' if n.avatar_url else ''}" for n in npcs]
        )
        ...
        if npc and len(npc.roles) > 0:
            ...
            self.add_field(name="Available to roles", value="\n".join(roles))

        self.add_field(
            name="Available NPCs (* = Has avatar)", value=npc_list, inline=False
        )
