...
class ShatterpointEmbed(discord.Embed):
    def __init__(
        ...
        player_list: bool = False,
        player: ShatterpointPlayer = None,
    ):
        super().__init__(
            title=f"Summary for shatterpoint: {shatterpoint.name}",
        ...
        scraped_channels = (
            [bot.get_channel(c).mention for c in shatterpoint.channels]
            if shatterpoint.channels
            else ["None"]
        )
        ...
        self.description = (
            f"**Base Chain Codes**: {shatterpoint.base_cc}\n"
        ...
        if player:
            ...
            self.add_field(
                name=f"Information for {member.display_name}",
        ...
        if override_players:
            self.add_field(
                ...
                value="\n".join(
                    [
                        f"{ZWSP3}{guild.get_member(p.player_id).mention} ({p.cc}, {p.renown_override if p.renown_override else 'NA'})"
        ...
        if player_list:
            ...
            for players in chunked_players:
                self.add_field(
                    ...
                    value="\n".join(
                        [
                            f"{ZWSP3}{member.mention if member else f'Unknown Member {p.player_id}'} ({p.cc:,}, {p.num_messages}, {len(p.characters)})"
                            for p in players
                            if (member := guild.get_member(p.player_id)) is not None
                        ]
                    ),
                    inline=False,
                )
...
class ShatterpointLogEmbed(discord.Embed):
    def __init__(self, shatterpoint: Shatterpoint):
        ...
        self.add_field(
            name="# of Entries", value=f"{len(shatterpoint.players)}", inline=False
        )

        self.add_field(name="# Characters", value=f"{num_characters}", inline=False)
