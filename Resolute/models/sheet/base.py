from Resolute.constants import (
    SAVE_NAMES,
    SKILL_MAP,
    SKILL_NAMES,
    STAT_ABBREVIATIONS,
...
from Resolute.helpers.general_helpers import camel_to_title, verbose_stat
...
class BaseStats:
    ...
    def get_mod(self, stat: str):
        ...
        if abbr_stat not in STAT_ABBREVIATIONS:
            raise G0T0CommandError(f"{stat} is not a valid stat.")
        return {
    ...
    def __getitem__(self, item):  # little bit hacky, but works
        if item not in STAT_NAMES:
            raise G0T0CommandError(f"{item} is not a stat.")
...
class Skill:
    def __init__(self, value, prof: float = 0, bonus: int = 0, adv=None):
        if prof not in (0, 0.5, 1, 2):
            raise G0T0CommandError("Prof must be 0, 0.5, 1, or 2.")
        self.value = value
        self.prof = prof
        self.bonus = bonus
        self.adv = adv
...
class Skills:
    ...
    def update(self, explicit_skills: dict):
        ...
        for skill, mod in explicit_skills.items():
            if skill not in self.skills:
                raise G0T0CommandError(f"{skill} is not a skill.")
    ...
    def __getattr__(self, item):
        if item not in self.skills:
            raise G0T0CommandError(f"{item} is not a skill.")
...
class Saves:
    ...
    def get(self, base_stat: str):
        ...
        if stat not in STAT_ABBREVIATIONS:
            raise G0T0CommandError(f"{base_stat} is not a base stat.")
    ...
    def update(self, explicit_saves: dict):
        ...
        for save, mod in explicit_saves.items():
            if save not in self.saves:
                raise G0T0CommandError(f"{save} is not a save.")
