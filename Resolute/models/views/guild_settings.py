...
class GuildSettingsUI(GuildSettings):
    ...
    async def guild_update_reset(
        self, _: discord.ui.Button, interaction: discord.Interaction
    ):
        await self.defer_to(_GuildResetView, interaction)

    @discord.ui.button(
        label="Update Stipends", style=discord.ButtonStyle.primary, row=1
    )
    async def guild_update_stipend(
        self, _: discord.ui.Button, interaction: discord.Interaction
    ):
...
class _GuildSettings2(GuildSettings):
    ...
    @discord.ui.button(
        label="New Player Messages", style=discord.ButtonStyle.primary, row=1
    )
    async def new_player_messages(
        self, _: discord.ui.Button, interaction: discord.Interaction
    ):
...
class _GuildResetView(GuildSettings):
    @discord.ui.button(
        label="Update Announcements", style=discord.ButtonStyle.primary, row=1
    )
    async def reset_announcements(
        self, _: discord.ui.Button, interaction: discord.Interaction
    ):
    ...
    async def reset_time(
        self, time: discord.ui.Select, interaction: discord.Interaction
    ):
    ...
    async def _before_send(self):
        time_options = [
            discord.SelectOption(
                label=f"{str(i).zfill(2)}:00 (GMT)",
                value=f"{str(i)}",
                default=True if self.guild._reset_hour == i else False,
            )
            for i in range(24)
        ]
        ...
        day_options = [
            discord.SelectOption(
                label=f"{day[0]}",
                value=f"{day[1]}",
                default=True if str(self.guild._reset_day) == day[1] else False,
            )
...
class _GuildStipendView(GuildSettings):
    ...
    async def guild_role_select(
        self, role: discord.ui.Select, interaction: discord.Interaction
    ):
        self.role = role.values[0]
        await self.refresh_content(interaction)

    @discord.ui.button(
        label="Add/Modify Stipend", style=discord.ButtonStyle.primary, row=2
    )
    async def guild_edit_stipend(
        self, _: discord.ui.Button, interaction: discord.Interaction
    ):
        stipend: RefWeeklyStipend = next(
            (s for s in self.guild.stipends if s.role_id == self.role.id),
    ...
    async def guild_remove_stipend(
        self, _: discord.ui.Button, interaction: discord.Interaction
    ):
        stipend: RefWeeklyStipend = next(
            (s for s in self.guild.stipends if s.role_id == self.role.id), None
        )

        if stipend is None:
            await interaction.channel.send(
                f"No stipend for `{self.role.name}`", delete_after=5
            )
    ...
    def __init__(self, guild: PlayerGuild):
        ...
        self.add_item(
            discord.ui.InputText(
                ...
                max_length=2,
                value=self.guild.max_characters,
            )
        )
        self.add_item(
            discord.ui.InputText(
                ...
                max_length=3,
                value=self.guild.handicap_cc,
            )
        )
        self.add_item(
            discord.ui.InputText(
                label="Diversion Limit (CC)",
                required=True,
                placeholder="Diversion Limit",
                max_length=3,
                value=self.guild.div_limit,
            )
        )
        self.add_item(
            discord.ui.InputText(
                label="Log Reward Points Threshold",
                required=False,
                placeholder="Log Reward Points Threshold",
                max_length=3,
                value=self.guild.reward_threshold,
            )
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        values = [
            ...
            (
                ...
                "max_characters",
                "Max characters must be a number!",
            ),
            (
                self.children[2].value,
                "handicap_cc",
                "Handicap amount must be a number!",
            ),
...
class GuildStipendModal(discord.ui.Modal):
    ...
    def __init__(self, stipend: RefWeeklyStipend, role: discord.Role):
        ...
        self.add_item(
            discord.ui.InputText(
                ...
                required=False,
                placeholder="Reason",
                value=self.stipend.reason,
            )
        )
        self.add_item(
            discord.ui.InputText(
                label="Leadership Stipend",
                required=False,
                max_length=5,
                placeholder="Leadership Stipend",
                value=self.stipend.leadership if self.stipend.leadership else False,
            )
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        leadership = (
            True
            if self.children[2] and get_positivity(self.children[2].value.lower())
            else False
        )
...
class GuildAnnouncementModal(discord.ui.Modal):
    ...
    def __init__(self, guild: PlayerGuild):
        ...
        announcement_string = ",".join(
            ['"' + x + '"' for x in self.guild.weekly_announcement]
        )
        ...
        self.add_item(
            discord.ui.InputText(
                ...
                max_length=3500,
                value=self.guild.reset_message,
            )
        )
        self.add_item(
            discord.ui.InputText(
                ...
                value=announcement_string,
            )
        )
        self.add_item(
            discord.ui.InputText(
                label="Ping on announcement?",
                required=False,
                placeholder="Ping on announcement?",
                value=f"{guild.ping_announcement}",
            )
        )
...
class ServerDateModal(discord.ui.Modal):
    ...
    def __init__(self, guild: PlayerGuild):
        ...
        self.add_item(
            discord.ui.InputText(
                ...
                max_length=50,
                value=(
                    self.guild.server_month.display_name
                    if self.guild.server_month is not None
                    else ""
                ),
            )
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        month = next(
            (
                month
                for month in self.guild.calendar
        ...
        if self.children[0] and month and self.children[2]:
            try:
                self.guild.server_date = self.guild.get_internal_date(
                    int(self.children[2].value),
                    self.guild.calendar.index(month) + 1,
                    int(self.children[0].value),
                )
            except:
                await interaction.channel.send(
                    embed=ErrorEmbed(f"Error setting server date"), delete_after=5
                )
...
class NewPlayerMessageModal(discord.ui.Modal):
    ...
    def __init__(self, guild: PlayerGuild):
        ...
        self.add_item(
            discord.ui.InputText(
                label="New discord.Member Greeting",
                style=discord.InputTextStyle.long,
                max_length=1000,
                required=False,
                value=self.guild.greeting,
            )
        )
        self.add_item(
            discord.ui.InputText(
                label="New Character Message",
                style=discord.InputTextStyle.multiline,
                max_length=500,
                required=False,
                value=self.guild.first_character_message,
            )
        )
