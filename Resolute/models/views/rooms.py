...
class RoomSettingsUI(RoomSettings):
    ...
    async def room_view(
        self, choice: discord.ui.Select, interaction: discord.Interaction
    ):
    ...
    async def room_add(self, _: discord.ui.Button, interaciton: discord.Interaction):
        ...
        await interaciton.channel.category.create_text_channel(
            response.name, reason=f"Room created by {self.owner.name}"
        )
    ...
    async def _before_send(self, interaction: discord.Interaction):
        ...
        view_options = [
            discord.SelectOption(
                ...
                value="1",
                description="Allow others to read and write",
                default=True if default == 1 else False,
            ),
            discord.SelectOption(
                label="Read Only Acccess",
                value="2",
                description="Allow others to just read",
                default=True if default == 2 else False,
            ),
            discord.SelectOption(
                label="No Access",
                value="3",
                description="Don't allow others to view/write",
                default=True if default == 3 else False,
            ),
        ]
...
class _RoomMoveUI(RoomSettings):
    ...
    async def move_up(self, _: discord.ui.Button, interaction: discord.Interaction):
        if self.current_position == 0:
            await interaction.channel.send(
                embed=ErrorEmbed(f"Channel is already in the top position"),
                delete_after=5,
            )
        else:
            await update_position(
                interaction.channel.category.channels,
    ...
    @discord.ui.button(
        label="Top", style=discord.ButtonStyle.primary, row=1, emoji="🔝"
    )
    async def move_top(self, _: discord.ui.Button, interaction: discord.Interaction):
        if self.current_position == 0:
            ...
            await update_position(
                interaction.channel.category.channels, self.current_position, 0
            )
    ...
    async def move_down(self, _: discord.ui.Button, interaction: discord.Interaction):
        if self.current_position == len(interaction.channel.category.channels) - 1:
            await interaction.channel.send(
                embed=ErrorEmbed(f"Channel is already in the lowest position"),
                delete_after=5,
            )
        else:
            await update_position(
    ...
    async def move_bottom(self, _: discord.ui.Button, interaction: discord.Interaction):
        if self.current_position == len(interaction.channel.category.channels) - 1:
            ...
            await update_position(
                interaction.channel.category.channels,
                self.current_position,
                len(interaction.channel.category.channels) - 1,
            )
    ...
    async def _before_send(self, interaction: discord.Interaction):
        self.current_position = interaction.channel.category.channels.index(
            interaction.channel
        )
    ...
    async def get_content(self) -> Mapping:
        return {
            "embed": None,
            "content": "Choose an option on where to move the channel: \n",
        }


async def update_position(
    channel_list: list[discord.TextChannel], old_position: int, new_position: int
) -> None:
